...
class Compiler
	...
	@@substitute_operator = [:substitute, :add_substitute, :sub_substitute, :mul_substitute, :div_substitute, :mod_substitute]
	...
	#******************** Token ********************
	def get_token()
		#puts "func_names = #{@func_names.join('|')}"
		#Keyword
		...
		#function call arguments(関数呼び出しの引数を取り出す)
			...
			@@code = $2 + $'
			puts "[Eat@func_call_arg] #{$1}"
			@look_func_call = false
			return [:func_call_arg, $1]
		#Numerical value
		elsif @@code =~ /\A\s*([0-9.]+)/
			@@code = $'
			puts "[Eat@numerical] #{$1}"
			return $1.to_f
		#String value
		elsif @@code =~ /\A\s*\"(.*?)\"/
			@@code = $'
		...
		#Variable name
			...
			@@code = $'
			puts "[Eat@variable_name] #{$1}"
			return [:variable, $1]
		#Whitespaces
		elsif @@code =~ /\A\s*\z/
			@@code = $'
			puts "[Empty@whitespace]"
			return nil
		end
		return :bad_token
	end
	...
	def declare_sentence()
		...
		puts "[declare] get name #{name}"
		if token == nil || token == :bad_token
			raise Exception, "Ungettable variable name"
		end
		equal = get_token()
		puts "[declare] get equal #{equal}"
		unless equal == :substitute # '='
			raise Exception, "expected \'=\' token"
		end
		value = condition() || scalar() || string_concat() || expression()
		puts "[declare] get value #{value}"
		if token == nil || token == :bad_token
	...
	def while_sentence()
		...
		token = get_token()
		unless token == :while
			puts "[repeat] not repeat_sentence #{token}"
			unget_token(token)
			return nil
		end
		token = get_token()
	...
	def def_sentence()
		...
		puts "[def] get token #{token}"
		name = get_token()
		unless name[0] == :func_name
			raise Exception, "Invalid function name"
		end
		puts "[def] get token #{name}"
		token = get_token()
		unless token == :lpar
			raise Exception, "Expected \'(\' token"
		end
		...
		while true
			...
			puts "[def] get arg #{arg}"
			puts "[def] Excess code = #{@@code}"
			arg_ary << arg
		end
		puts "Excess code = #{@@code}"
		token = get_token()
	...
	def call_sentence()
		...
		unless token.is_a?(Array)
			...
			puts "[call] get token #{token}"
			arg_ary = []
			while true
				arg = condition()  || scalar() || expression() || get_token()  || call_sentence()
				puts "[call] get arg #{arg}"
				sign = get_token()
				puts "[call] get sign #{sign}"
				if sign == :rpar
	...
	#-式
	def expression()
		puts "[operator_expression] in"
		result = term()
		while true
			token = get_token()
			puts "[operator_expression] get token #{token}"
	...
	#-項
	def term()
		puts "[operator_term] in"
		result = factor()
		while true
			token = get_token()
			puts "[operator_term] get token #{token}"
	...
	def factor()
		...
		puts "[operator_factor] get token #{token}"
		minusflg = 1

		if token == :sub
			puts "[operator_factor] this is sub!!"
			minusflg = -1
			token = get_token()
		end
	...
	#変数に対する代入演算
	def substitution()
		puts "[substitution] in"
		val1 = get_token()
		puts "[substitution] get token #{val1}"
		...
		val2 = nil
		operator = get_token()
		unless @@substitute_operator.include?(operator)
			unget_token(operator)
			return nil
		end
		val2 = scalar() || string_concat() || expression()
	...
	def condition()
		...
		operator = get_token()
		puts "[condition] get operator #{operator}"
	...
	#String concat
	def string_concat
		puts "[string_concat] in"
		str_concat = []
		s1 = get_token()
		puts "[string_concat] get s1 #{s1}"
		...
		while true
			op = get_token()
			puts "[string_concat] get op #{op}"
			unless op == :add
				unget_token(op)
				break
			end
			sn = get_token()
			puts "[string_concat] get sn #{sn}"
			unless s1.is_a?(String)
				raise Exception, "String + Other is Invalid"
			end
			str_concat << sn
		end
		puts "[string_concat] return #{[:string_concat, str_concat]}"
	...
	def scalar()
		...
		token = get_token()
		lookahead = get_token()
		puts "[scalar] get lookahead #{lookahead}"
		if @@operation_operators.include?(lookahead) || lookahead == :lpar #先読みして、演算でないか確かめる
			unget_token(lookahead)
			unget_token(token)
			return nil #スカラー値ではなく、演算
		end 
		unget_token(lookahead) #先読みして、大丈夫だったので戻しておく
		puts "[scalar] get token #{token}"
		if token == nil || token == :bad_token
			puts "nil or badtoken"
			return nil
	...
	def getVariable(name, where=nil)
		if where == nil
			...
			val = @func_arguments[[@func_name, name]]
		elsif where == "global"
	...
	def myEval(program)
		...
		if program.is_a?(Array)
			...
			when :condition
				...
				#puts "[condition] Judge #{op1} #{@@keywords.key(program[2])} #{op2}"
			...
			when :def
				...
				#puts "arg is #{program}"
				program[2].each {|arg|
					#puts "[def] store [#{program[1]}, #{arg}] => nil"
					@func_arguments.store([program[1], arg], nil)
				}
				#puts "[def] store program #{program[1]} => #{program[3]}"
			...
			when :substitution
				if program[2] == :substitute
					...
					#puts "val is #{"#{myEval(program[1])} #{@@keywords.key(program[2])[0]} #{myEval(program[3])}"}"
			...
			when :call
				...
				arg_names.each_with_index{|arg, idx|
					@func_arguments[[@func_name, arg]] = myEval(program[2][idx])
				}
				...
				#引数をnilに戻す
				arg_names.each {|arg|
					@func_arguments[[@func_name, arg]] = nil
				}
				@func_name = nil
			...
			#puts "[scalar] no execute #{program}"
			return program
		end	
	end
	...
	def start()
		...
		puts "local_variables = #{@local_variables}"
		puts "func_programs = #{@func_programs}"
		puts "func_arguments = #{@func_arguments}"
		puts "current_func_name = #{@current_func_name}"
		puts "look_func = #{@look_func}"
		puts "look_func_call = #{@look_func_call}"
	end
end
